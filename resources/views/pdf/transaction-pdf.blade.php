...
<body>
    ...
    <table class="info-table">
        <tr>
            <td style="width: 60%">
                <div class="label">SAYIN:</div>
                <div style="font-size: 11pt; font-weight: bold">{{ $customer->name }}</div>
    ...
    <table class="trans-table">
        ...
        <tbody>
            ...
            <tr style="background-color: #fff3cd">
                ...
                <td class="text-right">
                    @if($openingBalance > 0) {{ App\Common\Services\TransactionPdfService::formatAmount($openingBalance) }} @else - @endif
                </td>
                <td class="text-right">
                    @if($openingBalance < 0) {{ App\Common\Services\TransactionPdfService::formatAmount(abs($openingBalance)) }} @else - @endif
                </td>
                <td class="text-right font-bold {{ $openingBalance > 0 ? 'debt' : ($openingBalance < 0 ? 'credit' : '') }}">
                    {{ App\Common\Services\TransactionPdfService::formatAmount(abs($openingBalance)) }} 
            ...
            @foreach($transactions as $transaction)
                ...
                <tr>
                    ...
                    <!-- Borç Sütunu -->
                    <td class="text-right">
                        @if($isDebt) {{ App\Common\Services\TransactionPdfService::formatAmount($amount) }} @else - @endif
                    </td>
                    
                    <!-- Alacak Sütunu -->
                    <td class="text-right">
                        @if($isCredit) {{ App\Common\Services\TransactionPdfService::formatAmount($amount) }} @else - @endif
                    </td>
                    
                    <!-- Bakiye Sütunu -->
                    <td class="text-right font-bold {{ $runningBalance > 0 ? 'debt' : ($runningBalance < 0 ? 'credit' : '') }}">
                        {{ App\Common\Services\TransactionPdfService::formatAmount(abs($runningBalance)) }} 
    ...
    <div class="summary-box">
        ...
        <table style="width: 100%">
            <tr>
                <td>Toplam Borç:</td>
                <td class="text-right debt">{{ App\Common\Services\TransactionPdfService::formatAmount($totalDebt + ($openingBalance > 0 ? $openingBalance : 0)) }} TL</td>
            </tr>
            <tr>
                <td>Toplam Alacak:</td>
                <td class="text-right credit">{{ App\Common\Services\TransactionPdfService::formatAmount($totalCredit + ($openingBalance < 0 ? abs($openingBalance) : 0)) }} TL</td>
            </tr>
            <tr style="font-weight: bold; font-size: 11pt; border-top: 1px solid #999">
                ...
                <td class="text-right {{ $runningBalance > 0 ? 'debt' : ($runningBalance < 0 ? 'credit' : '') }}">
                    {{ App\Common\Services\TransactionPdfService::formatAmount(abs($runningBalance)) }} TL 
